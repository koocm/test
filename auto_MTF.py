...
while True:
    try:
        ...
        df=pyupbit.get_ohlcv(ticker="KRW-MFT",interval=interval,to=now,count=count)
        ...
        # print('05')
        # 위 두가지 조건을 만족하면 매수
        if (all(list) == True) & (ratio < 0.02) :
            krw = get_balance("KRW")
            if krw > 6000:
                upbit.buy_market_order("KRW-MFT", 6000)
        ...
        # print('06')
        # 수익률 1.5% 발생시 50% 매도
        if upbit.get_balance('MFT') > 100 :
            if float(upbit.get_balances()[1]['avg_buy_price']) > ((pyupbit.get_current_price(["KRW-XRP"]))*1.05) :
                upbit.sell_market_order("KRW-MFT",600)
