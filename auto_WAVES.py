...
while True:
    try:
        ...
        df=pyupbit.get_ohlcv(ticker="KRW-WAVES",interval=interval,to=now,count=count)
        ...
        # 현재 이동편균값이 현재가의 1.5% 범위 이내에 모여있는지 확인
        # print('04')
        ...
        if (all(list) == True) & (ratio < 0.015) :
            krw = get_balance("WAVES")
            if krw > 6000:
                upbit.buy_market_order("KRW-WAVES", 6000)
        ...
        # print('06')
        # 수익률 1.5% 발생시 50% 매도
        if upbit.get_balance('WAVES') > 0.2 :
            if float(upbit.get_balances()[3]['avg_buy_price']) > ((pyupbit.get_current_price(["KRW-WAVES"]))*1.05) :
                upbit.sell_market_order("KRW-WAVES",0.2)
